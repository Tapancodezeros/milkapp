...
class TransactionService {
    async buy(customerId, vendorId, quantity, type = 'purchase') {
        ...
        await AuditLogService.logAction({
            ...
            action: 'MILK_PURCHASE',
            entity: 'Transaction',
            entityId: transaction.id,
            details: { vendorId, vendorName: vendor.name, quantity: purchaseQuantity, amount, type }
        });
    ...
    async getTransactions(userId, role, options = {}) {
        ...
        if (search && role === UserRole.VENDOR) {
            where[Op.or] = [
        ...
        const populateVendorInfo = async (transactionRows) => {
            if (role === UserRole.CUSTOMER && transactionRows.length > 0) {
                const vendorIds = [...new Set(transactionRows.map(t => t.vendorId))];
    ...
    async verifyDelivery(transactionId, status, userId) {
        ...
        if (updatedTransaction) {
            const v = await Vendor.findByPk(updatedTransaction.vendorId, { attributes: ['name', 'phone'] });
            if (v) updatedTransaction.setDataValue('Vendor', v.toJSON ? v.toJSON() : v);
        }
    ...
    async pay(transactionId, customerId) {
        ...
        await AuditLogService.logAction({
            ...
            entity: 'Transaction',
            entityId: transactionId,
            details: { amountPaid: amountToPay, balance: customer.walletBalance }
        });
